...
void send_bio_to_device(struct work_struct *work)
{
	...
	if (ret != 0)
		pr_err("%s Received error code %d\n", __func__, ret);
}
...
static blk_qc_t raid_blk_submit_bio(struct bio *received_bio)
{
	...
	if (is_write == 1) {
		...
		bio_for_each_segment(bvec, received_bio, it) {
			...
			for (i = 0; i < 8; ++i) {
				...
				if (!correct_crc1 && !correct_crc2) {
					// Both disks are corrupt, free the data
					...
					// Signal a bio error
					pr_err("Both disks have corrupt data in sector %lld.\n", it.bi_sector + i);
					bio_io_error(received_bio);
		...
		if (correct_disk1) {
			rw_bio = create_wr_bio_from_pages(received_bio, &pages_rw_disk1, 1, 0);
			crc_bio = create_wr_bio_from_pages(received_bio, &pages_crc_disk1, 1, 1);
			...
			bio_put(rw_bio);
			bio_put(crc_bio);
		} else if (correct_disk2) {
			rw_bio = create_wr_bio_from_pages(received_bio, &pages_rw_disk2, 2, 0);
			crc_bio = create_wr_bio_from_pages(received_bio, &pages_crc_disk2, 2, 1);
