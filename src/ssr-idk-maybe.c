...
static struct bio *create_crc_bio(struct bio *model_bio, struct list_head *pages_list, int device, int is_write)
{
	...
	if (remaining_size < KERNEL_SECTOR_SIZE) {
		...
		int size_left_in_page = 4096 - offset;

		if (size_left_in_page >= remaining_size) {
			int ret = bio_add_page(bio, new_page, remaining_size, offset);
			pr_info("Add page bytes: %d\n", ret);
		} else {
			int ret = bio_add_page(bio, new_page, size_left_in_page, offset);
			pr_info("Add page bytes: %d\n", ret);

			remaining_size -= size_left_in_page;
			...
			ret = bio_add_page(bio, new_page, remaining_size, offset);
			pr_info("Add page bytes: %d\n", ret);
		}
	}
	pr_info("Total len: %d\n", total_len);
...
static blk_qc_t raid_blk_submit_bio(struct bio *received_bio)
{
	...
	debug_bio_data(bio_crc_disk1, "CRC1 data before:", 0);
	debug_bio_data(bio_crc_disk2, "CRC2 data before:", 0);
